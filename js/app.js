// Enhanced face detection module for real-time face swapping
...
let faceDetectionInterval = null; // Interval for face detection
...
function onOpenCvReady() {
    document.getElementById('statusMessage').textContent = 'OpenCV.js is loaded. Loading models...';
...
function loadFaceCascade() {
    ...
    utils.createFileFromUrl('haarcascade_frontalface_default.xml', 'models/haarcascade_frontalface_default.xml', () => {
        ...
        if (result) {
            document.getElementById('statusMessage').textContent = 'Face detection model loaded. Please allow webcam access.';
            
            // Enable webcam access
            startWebcam();
        } else {
            document.getElementById('statusMessage').textContent = 'Failed to load face detection model.';
        }
    });
}
...
function initializeVideoProcessing() {
    ...
    // Use videoWidth and videoHeight instead of height and width
            ...
            img.onload = function() {
                ...
                if (uploadedFaces.size() > 0) {
                    ...
                    document.getElementById('statusMessage').textContent = 
                        `Face detected in uploaded image. Click "Start Face Swap" to begin.`;
...
function startFaceSwap() {
    if (!faceCascade || !uploadedFace) {
        if (window.FaceSwapUI) {
            window.FaceSwapUI.updateStatus('error', 'Error: Face cascade or uploaded face not loaded.');
        } else {
            document.getElementById('statusMessage').textContent = 'Error: Face cascade or uploaded face not loaded.';
        }
        return;
    }
    ...
    if (window.FaceSwapUI) {
        ...
        document.getElementById('statusMessage').textContent = 'Face swapping in progress...';
    }
...
function stopFaceSwap() {
    ...
    if (window.FaceSwapUI) {
        ...
        document.getElementById('statusMessage').textContent = 'Face swapping stopped.';
    }
}
...
function processVideo() {
    ...
    try {
        ...
        try {
            ...
            if (window.FaceSwapUI) {
                window.FaceSwapUI.updateStatus('error', 'Error capturing video frame. Reinitializing...');
            }
        ...
        if (uploadedFace && faces.size() > 0) {
            ...
            if (smoothingAmount > 0) {
                ...
                cv.GaussianBlur(swappedResult, smoothedResult, new cv.Size(ksize, ksize), 0, 0);
            ...
            if (window.FaceSwapUI && faces.size() > 0) {
                window.FaceSwapUI.updateStatus('success', `Face swapping active with ${faces.size()} detected faces`);
            }
        } else {
            ...
            if (window.FaceSwapUI && faces.size() === 0 && uploadedFace) {
                window.FaceSwapUI.updateStatus('warning', 'No faces detected in webcam feed');
            }
        }
        ...
        if (window.FaceSwapUI) {
            window.FaceSwapUI.updateStatus('error', 'Error in video processing: ' + err.message + '. Reinitializing...');
        } else {
            document.getElementById('statusMessage').textContent = 'Error in video processing: ' + err.message + '. Reinitializing...';
        }
        ...
        request.onload = function(ev) {
            if (request.readyState === 4) {
                if (request.status === 200) {
                    ...
                    if (this.errorOutput) {
                        this.errorOutput.textContent = 'Failed to load ' + url + ' status: ' + request.status;
                    }
                }
            }
        };
...
window.onunload = function() {
    ...
    if (uploadedFace) uploadedFace.delete();
};
